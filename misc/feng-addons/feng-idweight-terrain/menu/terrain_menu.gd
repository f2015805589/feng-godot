...
const DirectoryWizard: Script = preload("res://addons/feng-idweight-terrain/menu/directory_setup.gd")
const ChannelPacker: Script = preload("res://addons/feng-idweight-terrain/menu/channel_packer.gd")
const LodBaker: Script = preload("res://addons/feng-idweight-terrain/menu/baker.gd")
...
func _on_menu_pressed(p_id: int) -> void:
	match p_id:
		...
		MENU_BAKE_NAV_MESH:
			baker.bake_nav_mesh()
		MENU_BRUSH_DIRECTORY:
			OS.shell_show_in_file_manager(ProjectSettings.globalize_path("res://addons/feng-idweight-terrain/brushes"))
		MENU_VIEW_LIVE_INFO_PANEL:
