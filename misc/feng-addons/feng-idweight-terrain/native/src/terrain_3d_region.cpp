...
// Terrain3DRegion, part 1 of 3: the record itself.

// One of three files that define a region. This one owns the four maps and their accessors, the
// validation that keeps them consistent (`sanitize_map()` and `validate_map_size()`), the region's
// height range, its location and instance count, and the ClassDB bindings.
//
// The other two: `terrain_3d_region_surface.cpp` (the R16 IdWeight surface map: what it is stored
// at, how it is created, how a legacy control map is converted into it, and the nearest resample
// all of that goes through) and `terrain_3d_region_io.cpp` (save, the map dictionary, duplicate
// and dump).
...
void Terrain3DRegion::_bind_methods() {
	...
	ClassDB::bind_method(D_METHOD("set_surface_map", "map"), &Terrain3DRegion::set_surface_map);
	...
	ADD_PROPERTY(PropertyInfo(Variant::OBJECT, "surface_map", PROPERTY_HINT_RESOURCE_TYPE, "Image", ro_flags), "set_surface_map", "get_surface_map");
