...
// Terrain3DInstancer's placement API: the instance data a caller writes and reads.

// One of three files that define Terrain3DInstancer. A region stores its foliage as
// `mesh_id -> cell -> [transforms, colours, modified]` (`terrain_3d_instancer.h`), and this half
// is everything that touches it: `add_instances()`, `remove_instances()`, `add_multimesh()`,
// `add_transforms()`, `append_location()`, `append_region()`, `update_transforms()` and the
// `get_closest_mesh_id()` query. Each of them ends by queueing the MMI work rather than doing it:
// `update_mmis()` in `terrain_3d_instancer.cpp` owns that.
//
// `_get_cell()` and `_get_usable_height()` are here because this is the only half that calls
...
void Terrain3DInstancer::update_transforms(const AABB &p_aabb) {
	...
	for (const Vector2i &region_loc : region_queue) {
		...
		for (const int &region_mesh_id : mesh_types) {
			...
			Dictionary c_locs;
			// Calculate step distance to ensure every cell is checked inside the bounds of the AABB size.
