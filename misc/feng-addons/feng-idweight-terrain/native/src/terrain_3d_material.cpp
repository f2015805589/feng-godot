...
// Terrain3DMaterial's resource: properties, uniforms, noise and save.
//
// One of two files that define Terrain3DMaterial. This one owns the material the
// terrain renders with: shader installation and uniform updates (including the VT
// samplers and the noise/gradient textures), initialization and teardown, the ~70
// property setters, save/load and the ClassDB bindings. `terrain_3d_material_shader.cpp`
// assembles the GLSL those uniforms belong to. Both carry the same include block so
// each compiles and reads on its own.
