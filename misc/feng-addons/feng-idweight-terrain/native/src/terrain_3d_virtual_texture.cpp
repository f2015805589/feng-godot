...
// The virtual texture runtime: the indirection texture, and everything that turns a page request
// into an entry in it.
//
// The physical side - the page atlas, the slot allocator and the reverse owner index - is
// terrain_3d_vt_page_pool.{h,cpp}, because both views allocate from one pool; this file is the
// per-view half. It owns the indirection image, whose mip chain is built by hand (averaging slot
// indices is meaningless: a coarser texel has to hold a real slot or the invalid marker), the
// sector block bookkeeping, and the request / lookup / release entry points the demand passes
// call. The addressing core the two halves share is terrain_vt.h, and the layout to read first
// is on the class in the header.
