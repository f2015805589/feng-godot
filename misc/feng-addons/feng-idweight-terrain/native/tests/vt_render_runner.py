...
def main():
    ...
    fixture = Path(tempfile.mkdtemp(prefix="terrain-vtrender-", dir=ROOT / "bin"))
        ...
        with log.open("w", encoding="utf-8") as out:
            ...
            if result.returncode == 0:
                result = subprocess.run(base + ["--rendering-method", "frp", "--rendering-driver", args.driver,
                    ...
                    str(Path(__file__).with_name("vt_render.gd").resolve()),
                    "--", "", str(shots)],
    ...
    return int(result.returncode != 0 or bool(errors) or "PASS surface virtual texture render integration" not in output)
