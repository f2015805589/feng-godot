...
def main() -> int:
    ...
    fixture = Path(tempfile.mkdtemp(prefix="terrain-vtclipmap-render-", dir=ROOT / "bin"))
    write_fixture(fixture)
    (fixture / "project.godot").write_text(
        'config_version=5\n[application]\nconfig/name="VT clipmap render tests"\n',
        encoding="utf-8",
    )
    env = os.environ.copy()
    ...
    log = fixture / "vtclipmaprender.log"
    shots = fixture / "shots"
    shots.mkdir()
            ...
            if result.returncode == 0:
                result = subprocess.run(
                    ...
                    + [
                        ...
                        "-10000,-10000",
                        "--script",
                        str(Path(__file__).with_name("vt_clipmap_render.gd").resolve()),
                        "--",
                        "",
                        str(shots),
                    ],
