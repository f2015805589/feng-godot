...
def main():
        ...
        with log.open("w", encoding="utf-8") as out:
            ...
            if result.returncode == 0:
                result = subprocess.run(base + ["--rendering-method", "frp", "--rendering-driver", args.driver,
                    ...
                    str(Path(__file__).with_name("vt_sparse.gd").resolve())],
    ...
    for line in output.splitlines():
        if line.startswith(("PASS", "REGRESSION", "ERROR:", "SCRIPT ERROR:", "VTSVT")):
            print(line)
    print(f"EXIT={result.returncode} ERRORS={len(errors)} LOG={log}")
    return int(result.returncode != 0 or bool(errors) or "PASS sparse virtual texture far field" not in output)
