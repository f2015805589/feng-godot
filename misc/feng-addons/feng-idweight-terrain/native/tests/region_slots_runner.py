...
def main():
        ...
        with log.open("w", encoding="utf-8") as out:
            ...
            if result.returncode == 0:
                result = subprocess.run(base + ["--rendering-method", "frp", "--rendering-driver", args.driver,
                    "--resolution", "480x480", "--position", "-10000,-10000", "--script",
                    str(Path(__file__).with_name("region_slots.gd").resolve())],
    ...
    for line in output.splitlines():
        if line.startswith(("PASS", "SLOTS", "REGRESSION", "ERROR:", "SCRIPT ERROR:")):
            print(line)
    print(f"EXIT={result.returncode} ERRORS={len(errors)} LOG={log}")
    return int(result.returncode != 0 or bool(errors) or "PASS region layer slots" not in output)
