...
def main():
    ...
    fixture = Path(tempfile.mkdtemp(prefix="terrain-vtroot-", dir=ROOT / "bin"))
    write_fixture(fixture)
    (fixture / "project.godot").write_text(
        'config_version=5\n[application]\nconfig/name="VT root budget tests"\n',
        encoding="utf-8",
    )
    env = os.environ.copy()
            ...
            if result.returncode == 0:
                result = subprocess.run(
                    ...
                    + [
                        ...
                        "-10000,-10000",
                        "--script",
                        str(Path(__file__).with_name("vt_root_budget.gd").resolve()),
                    ],
                    env=env,
                    stdout=out,
                    stderr=subprocess.STDOUT,
    ...
    for line in output.splitlines():
        if line.startswith(("PASS", "REGRESSION", "ERROR:", "SCRIPT ERROR:", "VTROOT")) and not is_environmental_error(line):
            print(line)
    print(f"EXIT={result.returncode} ERRORS={len(errors)} LOG={log}")
    return int(result.returncode != 0 or bool(errors) or "PASS bounded SVT root and detail scheduling" not in output)
