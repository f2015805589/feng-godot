...
def main() -> int:
    ...
    fixture = Path(tempfile.mkdtemp(prefix="terrain-vtvisibility-", dir=ROOT / "bin"))
    write_fixture(fixture)
    (fixture / "project.godot").write_text(
        'config_version=5\n[application]\nconfig/name="VT visibility tests"\n',
        encoding="utf-8",
    )
    shots = fixture / "shots"
    shots.mkdir()
            ...
            if result.returncode == 0:
                result = subprocess.run(
                    ...
                    + [
                        ...
                        "-10000,-10000",
                        "--script",
                        str(Path(__file__).with_name("vt_visibility.gd").resolve()),
                        "--",
                        "",
                        str(shots),
                    ],
    ...
    for line in output.splitlines():
        if line.startswith(("PASS", "REGRESSION", "ERROR:", "SCRIPT ERROR:", "VT_VISIBILITY")) and not is_environmental_error(line):
            print(line)
    print(f"EXIT={result.returncode} ERRORS={len(errors)} LOG={log}")
    return int(
        result.returncode != 0
        or bool(errors)
        or "PASS camera-visible AVT focus and nearest SVT page demand" not in output
    )
