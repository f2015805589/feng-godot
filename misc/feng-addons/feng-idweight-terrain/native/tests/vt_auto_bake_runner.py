...
def main():
    ...
    fixture = Path(tempfile.mkdtemp(prefix="terrain-vtautobake-", dir=ROOT / "bin"))
    write_fixture(fixture)
    (fixture / "vt_render_base.gd").write_text(Path(__file__).with_name("vt_render.gd").read_text(encoding="utf-8"), encoding="utf-8")
    (fixture / "project.godot").write_text('config_version=5\n[application]\nconfig/name="VT render tests"\n', encoding="utf-8")
    shots = fixture / "shots"
    shots.mkdir()
    env = os.environ.copy()
        ...
        with log.open("w", encoding="utf-8") as out:
            ...
            if result.returncode == 0:
                result = subprocess.run(base + ["--rendering-method", "frp", "--rendering-driver", args.driver,
                    ...
                    str(Path(__file__).with_name("vt_auto_bake.gd").resolve()),
                    "--", "", str(shots)],
    ...
    for line in output.splitlines():
        if line.startswith(("PASS", "REGRESSION", "ERROR:", "SCRIPT ERROR:")) and not is_environmental_error(line):
            print(line)
    print(f"EXIT={result.returncode} ERRORS={len(errors)} LOG={log}")
    return int(result.returncode != 0 or bool(errors) or "PASS automatic incremental SVT baking" not in output or "PASS cross-process SVT content identity" not in output)
