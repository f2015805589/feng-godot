...
def main():
    ...
    fixture = Path(tempfile.mkdtemp(prefix="terrain-vtmaterial-", dir=ROOT / "bin"))
        ...
        with log.open("w", encoding="utf-8") as out:
            ...
            if result.returncode == 0:
                result = subprocess.run(base + ["--rendering-method", "frp", "--rendering-driver", args.driver,
                    ...
                    str(Path(__file__).with_name("vt_material.gd").resolve()),
                    "--", "", str(shots)],
    ...
    return int(result.returncode != 0 or bool(errors) or "PASS AVT material and SVT persistence integration" not in output)
