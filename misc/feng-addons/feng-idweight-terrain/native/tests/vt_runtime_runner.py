...
def main():
    ...
    fixture = Path(tempfile.mkdtemp(prefix="terrain-vt-", dir=ROOT / "bin"))
    write_fixture(fixture)
    (fixture / "project.godot").write_text('config_version=5\n[application]\nconfig/name="VT runtime tests"\n', encoding="utf-8")
    env = os.environ.copy()
        ...
        with log.open("w", encoding="utf-8") as out:
            ...
            if result.returncode == 0:
                result = subprocess.run(base + ["--rendering-method", "frp", "--rendering-driver", args.driver,
                    ...
                    str(Path(__file__).with_name("vt_runtime.gd").resolve())],
    ...
    return int(result.returncode != 0 or bool(errors) or "PASS virtual texture runtime" not in output)
