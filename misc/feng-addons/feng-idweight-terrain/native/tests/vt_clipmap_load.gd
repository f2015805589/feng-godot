...
# user's own path: the near material group on `Clipmap`, `vt_clipmap_size = 256`, four units,
...
#
# It also drives the *same* readings through the atlas mechanism, which is now the other
# *implementation* of the one `Clipmap` delivery (`vt_clipmap_implementation = Atlas`) rather than a
# delivery of its own, so the before/after table is produced by one script on one build and the
# comparison is the mechanism's own counters rather than two runs of two programs.
...
# What only the selected implementation can say - the atlas's cell table and rolling counters, the LOD
# ring's level reports - is nested under the group entry's `layout`: the implementation payload.
func entry_impl(p_entry: Dictionary) -> Dictionary:
	return p_entry.get("layout", {})

func cell_table_of(p_entry: Dictionary) -> Array:
	return (entry_impl(p_entry).get("layout", {}) as Dictionary).get("cells", [])

func current_cells_of(p_entry: Dictionary) -> int:
	var count := 0
	for value: Variant in cell_table_of(p_entry):
		count += 1 if bool((value as Dictionary).get("current", false)) else 0
	return count

func baked_cells_of(p_entry: Dictionary) -> int:
	return int(entry_impl(p_entry).get("baked_cells", 0))

func valid_units(p_entry: Dictionary) -> int:
	var count := 0
	for report: Dictionary in (p_entry.get("unit_reports", []) as Array):
...
# The coarse layer is settled when every unit is current and no bake is outstanding. A group with no
# layer reports no `units`, which is settled by construction.
func ring_settled() -> bool:
	var entry := clip_entry()
	if not bool(entry.get("configured", false)):
		return true
	if int(entry.get("units", 0)) <= 0:
		return true
	return valid_units(entry) >= int(entry.get("units", 0)) \
...
func window(label: String, p_settled: Callable, p_cap: int) -> Dictionary:
	...
	while frames < p_cap:
		...
		bake_dispatches = int(entry_impl(entry).get("bake_dispatches", 0))
		last_valid = valid_units(entry)
...
# The atlas mechanism's own load window. It is driven the way the LOD mechanism's tests drive the
# ring - through the layer's one entry, one call a simulated frame - because the atlas is an
# implementation of the one mechanism before it is a delivery. The near material ring is put on
# `Direct` for the window so the two implementations are not ticked at once and the cost measured
# here is the atlas's own.
func atlas_entry() -> Dictionary:
	# One `Clipmap` delivery now: the ring and the atlas share the group entry, and
	# `vt_clipmap_implementation` decides which storage answers it.
	return clip_entry()
...
func atlas_settled() -> bool:
	...
	return int(entry.get("pending_jobs", 0)) == 0 \
			and current_cells_of(entry) >= cell_table_of(entry).size()
...
func material_atlas_settled() -> bool:
	...
	return int(entry.get("pending_jobs", 0)) == 0 \
			and current_cells_of(entry) >= cell_table_of(entry).size() \
			and int(entry.get("pending_bake_rects", 0)) == 0 \
			and baked_cells_of(entry) >= current_cells_of(entry)

# The near field is the finest unit's own cells: the blocks the ground under the camera reads. It is
# the moment the atlas is *usable*, and it is deliberately a separate reading from the whole grid
# being current, because the dependency order fills the finest unit first.
func atlas_near_settled() -> bool:
	var entry := atlas_entry()
	if entry.is_empty():
		return true
	var rings: Array = entry_impl(entry).get("ring_reports", [])
	if rings.is_empty():
		return false
...
func atlas_window(label: String, p_settled: Callable, p_cap: int) -> Dictionary:
	...
	while frames < p_cap:
		...
		var made := terrain.debug_update_vt_clipmap(MATERIAL)
		...
		var entry := atlas_entry()
		upload_bytes = int(entry.get("upload_bytes", 0))
		block_uploads = int(entry_impl(entry).get("block_uploads", 0))
		current = current_cells_of(entry)
...
# The **delivery path's** own window: a cell names `Clipmap` while the implementation is `Atlas`, so
# the tick's phase drives the atlas and this only *observes* - no `debug_update_vt_clipmap()` call in
# the loop. `clipmap_ms` is the tick's whole clipmap phase, which is the number a user's frame pays;
# the per-entry counters are the atlas's own.
func delivery_window(label: String, p_settled: Callable, p_cap: int) -> Dictionary:
	...
	while frames < p_cap:
		...
		ms += frame_ms
		peak_ms = maxf(peak_ms, frame_ms)
		produced += int(settings().get("clipmap_produced_texels", 0))
		var entry := atlas_entry()
		upload_bytes = int(entry.get("upload_bytes", 0))
		block_uploads = int(entry_impl(entry).get("block_uploads", 0))
		current = current_cells_of(entry)
		baked = baked_cells_of(entry)
		pending = int(entry.get("pending_jobs", 0))
		print("CLIPMAP_LOAD_ROW %s frame=%d atlas_ms=%.3f produced=%d upload_bytes=%d block_uploads=%d current=%d baked=%d pending=%d" % [
			label, frames, frame_ms, int(settings().get("clipmap_produced_texels", 0)),
...
func atlas_roll_report(label: String) -> Dictionary:
	var entry := atlas_entry()
	var payload := entry_impl(entry)
	var report := {
		"label": label,
		"scroll_events": int(payload.get("scroll_events", 0)),
		"blocks_loaded": int(payload.get("blocks_loaded", 0)),
		"blocks_retained": int(payload.get("blocks_retained", 0)),
		"last_loaded": int(payload.get("last_scroll_loaded", 0)),
		"last_retained": int(payload.get("last_scroll_retained", 0)),
		"cells": cell_table_of(entry).size(),
...
func setup() -> void:
	...
	# The user's path: the near material group on Clipmap, the shipped shape of the layer, and the LOD
	# implementation explicitly selected (the atlas is the other implementation of the same delivery).
...
func run() -> void:
	...
	# The atlas mechanism, which is the other *implementation* of the one delivery now: select it, then
	# run the same two windows through the layer's own counters. The material ring is put on `Direct`
	# first so the two implementations are not driven at once and the number measured is the atlas's
	# own.
	if terrain.has_method("debug_update_vt_clipmap") and terrain.has_method("has_vt_clipmap_layer"):
		terrain.vt_clipmap_implementation = ATLAS
		var built: int = terrain.call("debug_update_vt_clipmap", MATERIAL)
		...
		if built >= 0:
			...
			print("CLIPMAP_LOAD_ATLAS_PHASE frames=%d produced=%d upload_delta=%d scrolls=%d" % [
				...
				int(entry_impl(after_phase).get("scroll_events", 0))])
	...
	# ---- D. The delivery path ---------------------------------------------------------------------
	# The measurement the user's report is about: a *cell* names `Clipmap` with the atlas implementation
	# selected, the tick's own phase drives it, and this only observes - no `debug_update_vt_clipmap()`
	# call in the loop. The near material's whole chain is here: the atlas's first fill (its cells
	# current *and* their baked rects acknowledged), the detail layer's own fill, and the two scrolls.
	terrain.vt_clipmap_implementation = ATLAS
	terrain.vt_delivery_near_material = CLIPMAP
	await settle(6)
	camera.position = Vector3(0.0, 12.0, 14.0)
	...
	print("CLIPMAP_LOAD_DELIVERY_TABLE near_frames=%d near_ms=%.3f first_frames=%d first_ms=%.3f first_peak_ms=%.3f first_delta_bytes=%d first_delta_blocks=%d detail_frames=%d detail_ms=%.3f chain_frames=%d" % [
		...
		int(d_first["block_uploads"]) - int(entry_impl(before_delivery).get("block_uploads", 0)),
