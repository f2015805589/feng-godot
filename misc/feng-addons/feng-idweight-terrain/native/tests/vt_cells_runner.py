...
def main():
    ...
    fixture = Path(tempfile.mkdtemp(prefix="terrain-vtcells-", dir=ROOT / "bin"))
        ...
        with log.open("w", encoding="utf-8") as out:
            ...
            if result.returncode == 0:
                result = subprocess.run(base + ["--rendering-method", "frp", "--rendering-driver", args.driver,
                    ...
                    str(Path(__file__).with_name("vt_cells_mix.gd" if args.mix else "vt_cells.gd").resolve()),
                    "--", "", str(shots)],
    ...
    return int(result.returncode != 0 or bool(errors) or "PASS SVT cell sources bake" not in output or "PASS SVT cell sources reload" not in output)
