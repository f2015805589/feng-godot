...
def main():
        ...
        with log.open("w", encoding="utf-8") as out:
            ...
            if result.returncode == 0:
                result = subprocess.run(base + ["--rendering-method", "frp", "--rendering-driver", args.driver,
                    ...
                    str(Path(__file__).with_name("texture_compression.gd").resolve())],
                    env=env, stdout=out, stderr=subprocess.STDOUT, timeout=180)
    ...
    for line in output.splitlines():
        if line.startswith(("CODEC=", "PASS", "ERROR:", "SCRIPT ERROR:")):
            print(line)
    print(f"EXIT={result.returncode} ERRORS={len(errors)} LOG={log}")
    return int(result.returncode != 0 or bool(errors) or "PASS all array compression formats" not in output)
